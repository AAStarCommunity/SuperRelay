...
use super_relay_gateway::signature_validator::{
    SignatureValidator, SignatureFormat, SignatureComponents
};
use super_relay_gateway::validation::{DataIntegrityChecker, ValidationConfig};
use super_relay_gateway::validation::ValidationSeverity;
...
async fn test_malleable_signature_detection() {
    ...
    assert!(
        result.security_issues.iter().any(|issue| issue.contains("malleable")),
        "应该明确指出malleable问题"
    );
...
async fn test_invalid_r_component() {
    ...
    assert!(
        result.security_issues.iter().any(|issue| issue.contains("Invalid r component")),
        "应该检测到无效r组件"
    );
...
async fn test_invalid_s_component() {
    ...
    assert!(
        result.security_issues.iter().any(|issue| issue.contains("Invalid s component")),
        "应该检测到无效s组件"
    );
...
async fn test_invalid_v_component() {
    ...
    for v_value in invalid_v_values {
        ...
        assert!(
            result.security_issues.iter().any(|issue| issue.contains("Invalid recovery id")),
...
async fn test_lenient_vs_strict_mode() {
    ...
    // 严格模式应该拒绝
    let strict_result = strict_validator.validate_signature(&signature).await.unwrap();
    assert!(!strict_result.is_valid, "严格模式应该拒绝malleable签名");

    // 宽松模式应该接受但有警告
    let lenient_result = lenient_validator.validate_signature(&signature).await.unwrap();
...
async fn test_integrated_signature_validation() {
    ...
    // 创建有效签名
    let mut valid_signature = vec![0u8; 65];
...
async fn test_signature_validation_performance() {
    ...
    // 性能要求：每次验证应该在1ms以内
    assert!(avg_time.as_millis() < 1, "签名验证性能应该 < 1ms，实际: {:?}", avg_time);
